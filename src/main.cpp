...
// -----------------------------------------------------------------------------
// 認証情報のオーバーライド (include/secrets.h が存在する場合に読み込まれる)
// -----------------------------------------------------------------------------
...
constexpr long TIMEZONE_OFFSET_SEC = 9L * 3600L; // JST (+9時間)
...
// タイミング定数
// -----------------------------------------------------------------------------
constexpr uint32_t TICK_INTERVAL_MS = 10;            // 10ミリ秒ティック
...
// デバイス上でのキャリブレーション後、必要に応じてこれらの中点を調整する。
constexpr uint16_t ADC_THRESHOLD_BTN1_BTN2 = 840;  // 10kΩと47kΩの中点
constexpr uint16_t ADC_THRESHOLD_BTN2_BTN3 = 1480; // 47kΩと68kΩの中点
constexpr uint16_t ADC_THRESHOLD_BTN3_IDLE = 2850; // 68kΩとオープンの中点
...
private:
  ...
  void maybeHandleDayCutover()
  {
    ...
    // remaining_ms_ は既に残りを反映している（一部消費されたため）
    elapsed_ms_in_segment_ = 0;
...
static void drawWifiIndicator(Adafruit_ST7789 &display, int16_t x, int16_t y, bool connected)
{
  ...
  // 3本のシグナルバーを描画; 接続時は塗りつぶし、オフライン時は輪郭で一目でわかるように。
  for (int i = 0; i < 3; ++i)
  {
